...
![Processes running as root](screenshots/10_ps-aux_root.PNG)
...
Looks like it is running on TCP 5801 and 5901 on localhost.  That is great but we do not have credentials to connect.  It is possible that it will not require credentials, but that is unlikely.  Looking at the manpage for vncviewer[1], we see that it can take a password file generated by vncpasswd.  THe man page for vncpasswd[2] says that you can specify a file with the -rfbauth flag.  The output from ps got off before, so we can see it with pgrep (which perhaps we should have done before):
```bash
...
![vnc as root](screenshots/vnc_root.png)
